...
namespace FitnessDesktop
{
    public class DisplayRootRegistry
    {
        ...
        public void CloseWindow(int HashCode)
        {
            foreach (Window openWindow in Application.Current.Windows)
            {
                if (openWindow.DataContext.GetHashCode() == HashCode)
                    openWindow.Close();
            }
        }
        public static object KeyByValue(Dictionary<object, Window> dict, Window val)
        {
            object key = default(object);
            foreach (KeyValuePair<object, Window> pair in dict)
            {
                if (pair.Value == val)
                {
                    key = pair.Key;
